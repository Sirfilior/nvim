...
Util.pick._open = function(command, opts)
  opts = opts or {}
...
local function symbols_filter(entry, ctx)
  if ctx.symbols_filter == nil then
    ctx.symbols_filter = require("util").ui.get_kind_filter(ctx.bufnr) or false
  end
...
return {
  ...
  },
  {
    ...
    keys = {
      ...
      { "<leader><space>", Util.pick("auto"), desc = "Find Files (Root Dir)" },
      -- find
      { "<leader>fb", "<cmd>FzfLua buffers sort_mru=true sort_lastused=true<cr>", desc = "Buffers" },
      { "<leader>fc", Util.pick.config_files(), desc = "Find Config File" },
      { "<leader>ff", Util.pick("auto"), desc = "Find Files (Root Dir)" },
...

  {
    ...
    opts = function()
      local Keys = require("main.plugins.lsp.keymaps").get()
      -- stylua: ignore
